      ...
      <div style={{ background: '#fff', borderRadius: 18, width: 420, maxWidth: '100%', boxShadow: '0 24px 60px rgba(20,40,80,.3)', overflow: 'hidden', animation: 'scIn .24s ease both' }}>
        <div style={{ padding: '24px 24px 16px', textAlign: 'center' }}>
          <div style={{ display: 'flex', justifyContent: 'center', gap: 10, alignItems: 'center', marginBottom: 14 }}>
            <span style={{ fontSize: 11.5, fontWeight: 700, color: '#fff', background: fromPlan.color, padding: '5px 11px', borderRadius: 7 }}>{fromPlan.label}</span>
            <span style={{ color: '#c3c8d0', fontSize: 14 }}>→</span>
            <span style={{ fontSize: 11.5, fontWeight: 700, color: '#fff', background: toPlan.color, padding: '5px 11px', borderRadius: 7 }}>{toPlan.label}</span>
          </div>
          <h2 style={{ margin: 0, fontSize: 16, fontWeight: 700 }}>月額料金が変わります</h2>
          <p style={{ margin: '8px 0 0', fontSize: 12.5, color: '#8a909a', lineHeight: 1.7 }}>
            {unitLabel}を追加すると、月額 <span style={{ fontWeight: 700, color: '#3a4150' }}>¥{toPlan.price.toLocaleString('ja-JP')}</span> のお支払いが発生します。
          </p>
        </div>
